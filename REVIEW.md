...
ограничения относятся к продуктовым требованиям: настоящий почтовый провайдер, его
идемпотентность, retry policy, scheduler, права на персональные данные и полный resource
control недоверенных XLSX.
...
| Большие файлы | Выполнено в заданном масштабе | `read_only=True`, пакет 1..500; библиотечные метаданные всё ещё могут занимать память |
...
которую код читает целиком, ограничена 10 MiB. Это закрывает подмену первого отсутствующего
листа следующим листом и очевидные XML entity attacks. Общий ZIP/worksheet/shared strings
пока не имеют единого распакованного лимита — это документированная граница.
...
Размер пакета ограничен 500: максимум 6000 параметров на statement. В памяти находится
текущий пакет, хотя openpyxl вправе дополнительно хранить shared strings и метаданные.

## Модель и схема

`external_id` имеет UNIQUE. `status` индексирован для выборки очереди. `user_id` — внешний
положительный bigint, а не связь с локальным пользователем. `claim_token` отделяет конкретную
попытку конкретного владельца; `claimed_at` образует lease; `attempts` и `last_error` дают
минимальную операционную диагностику без текста исключения.

Миграция `0002` закрепляет `user_id > 0` и допустимые комбинации `status`, `claim_token`,
`claimed_at`, `sent_at`. База отклоняет, например, `sent` без `sent_at` и `processing` без
token/lease. CHECK не заменяет service: только условный UPDATE с собственным token определяет
право перехода, а БД проверяет форму конечного состояния.
...
Захват — compare-and-set:

1. выбирается кандидат до `max_id`, зафиксированного в начале запуска;
2. условный UPDATE проверяет, что кандидат всё ещё `pending`, разрешённый `failed` или имеет
   истёкшую `processing` lease;
3. победитель пишет UUID token и увеличивает attempts;
4. после транспорта финальный UPDATE фильтруется по PK, `processing` и тому же token.

Два процесса могут прочитать одного кандидата, но только один UPDATE изменит строку. Старый
владелец после истечения lease не способен записать результат поверх нового владельца.
Задержка и транспорт выполняются без открытой транзакции и без удержания row lock.
...
является результатом отправки. Ошибка `write()` приводит к `failed`. Диагностические логи
остаются best effort и не заменяют уже сохранённый бизнес-результат собственной ошибкой.
...
почтовым API; нужен provider idempotency key или transactional outbox с идемпотентным
потребителем.
...
- успех, failure, retry, limit, lease, stolen/lost claim и DB failures отправителя;
...
| Проверка | Результат |
|---|---|
| PostgreSQL 16.14 + Python 3.12 | 48 тестов, OK, 1.062 с |
| PostgreSQL 16.14 + Python 3.10 | 48 тестов, OK, 1.112 с |
| Отдельный concurrency test | OK |
...
2. Для реальной почты определить timeout/idempotency контракт провайдера, хранить provider ID
   и различать подтверждённый отказ с неизвестным результатом.
3. Добавить retry policy только после определения временных/постоянных ошибок: `max_attempts`,
   `next_attempt_at`, backoff+jitter и операторский просмотр failed.
4. Если транспорт может длиться близко к пяти минутам, добавить heartbeat/renewal lease.
5. Перед API определить tenant/source scope, согласие на рассылку, права, retention и backup.
6. Перед применением `0002` к существующей production-базе проверить старые строки на новые
   constraints; в тестовом проекте миграция проверена на чистой PostgreSQL.
7. При недоверенных больших файлах ограничить ZIP compressed/uncompressed size, число строк
   и колонок и измерить память на реалистичном наборе.
8. Менять CAS-очередь на `select_for_update(skip_locked=True)` только если профилирование
   покажет contention на выборе кандидатов.
